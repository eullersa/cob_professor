...
function Form({text, button, sendForm, inputs, defaultState, setCourses, photoModel, id}) {
    ...
    await sendForm(token, data, photoModel)
      .then(response => {
        if(response.success === false) {
          ...
          sendBack = response[`${photoModel.replace('class/', '')}s`]
        }
      })
      .catch(error => console.error(error))

    if(profilePic) {
      await sendImage(profilePic.image, profilePic.type, photoModel, token, {name: data.name, course: id})
      .then((response) => {
                      ...
                      <div className="form-group" key={i}>
                        {(() => {
                          if(input.type === 'file') {
                            ...
                            return (
                              <input type="text" onChange={handleChange(input.setInput)} className="form-control" placeholder={input.placeholder} required />
                            )
                          }
                        })()}
                      </div>
